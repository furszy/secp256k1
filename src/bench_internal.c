...
static void bench_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        secp256k1_sha256_write(&sha, data->data, 32, secp256k1_get_sha256_transform(data->ctx));
        secp256k1_sha256_finalize(&sha, data->data, secp256k1_get_sha256_transform(data->ctx));
    }
}
...
static void bench_hmac_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_hmac_sha256_initialize(&hmac, data->data, 32, secp256k1_get_sha256_transform(data->ctx));
        secp256k1_hmac_sha256_write(&hmac, data->data, 32, secp256k1_get_sha256_transform(data->ctx));
        secp256k1_hmac_sha256_finalize(&hmac, data->data, secp256k1_get_sha256_transform(data->ctx));
    }
}
...
static void bench_rfc6979_hmac_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        secp256k1_rfc6979_hmac_sha256_initialize(&rng, data->data, 64, secp256k1_get_sha256_transform(data->ctx));
        secp256k1_rfc6979_hmac_sha256_generate(&rng, data->data, 32, secp256k1_get_sha256_transform(data->ctx));
    }
}
