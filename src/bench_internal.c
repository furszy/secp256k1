...
static void bench_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        secp256k1_sha256_write(&sha, data->data, 32, data->ctx->hash_context.fn_sha256_transform);
        secp256k1_sha256_finalize(&sha, data->data, data->ctx->hash_context.fn_sha256_transform);
    }
}
...
static void bench_hmac_sha256(void* arg, int iters) {
    ...
    for (i = 0; i < iters; i++) {
        ...
        secp256k1_hmac_sha256_write(&hmac, data->data, 32, data->ctx->hash_context.fn_sha256_transform);
        secp256k1_hmac_sha256_finalize(&hmac, data->data, data->ctx->hash_context.fn_sha256_transform);
    }
}
