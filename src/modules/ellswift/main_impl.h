...
static void secp256k1_ellswift_prng(const secp256k1_context *ctx, unsigned char* out32, const secp256k1_sha256 *hasher, uint32_t cnt) {
    ...
    buf4[2] = cnt >> 16;
    buf4[3] = cnt >> 24;
    secp256k1_sha256_write(&hash, buf4, 4, ctx->hash_context.fn_sha256_transform);
    secp256k1_sha256_finalize(&hash, out32, ctx->hash_context.fn_sha256_transform);
...
int secp256k1_ellswift_encode(const secp256k1_context *ctx, unsigned char *ell64, const secp256k1_pubkey *pubkey, const unsigned char *rnd32) {
    ...
    if (secp256k1_pubkey_load(ctx, &p, pubkey)) {
        ...
        secp256k1_sha256_write(&hash, p64, sizeof(p64), ctx->hash_context.fn_sha256_transform);
        secp256k1_sha256_write(&hash, rnd32, 32, ctx->hash_context.fn_sha256_transform);
...
int secp256k1_ellswift_create(const secp256k1_context *ctx, unsigned char *ell64, const unsigned char *seckey32, const unsigned char *auxrnd32) {
    ...
    secp256k1_sha256_write(&hash, seckey32, 32, ctx->hash_context.fn_sha256_transform);
    secp256k1_sha256_write(&hash, zero32, sizeof(zero32), ctx->hash_context.fn_sha256_transform);
    secp256k1_declassify(ctx, &hash, sizeof(hash)); /* private key is hashed now */
    if (auxrnd32) secp256k1_sha256_write(&hash, auxrnd32, 32, ctx->hash_context.fn_sha256_transform);
...
static int ellswift_xdh_hash_function_prefix_impl(const secp256k1_context *ctx, unsigned char *output, const unsigned char *x32, const unsigned char *ell_a64, const unsigned char *ell_b64, void *data) {
    ...
    secp256k1_sha256_write(&sha, data, 64, ctx->hash_context.fn_sha256_transform);
    secp256k1_sha256_write(&sha, ell_a64, 64, ctx->hash_context.fn_sha256_transform);
    secp256k1_sha256_write(&sha, ell_b64, 64, ctx->hash_context.fn_sha256_transform);
    secp256k1_sha256_write(&sha, x32, 32, ctx->hash_context.fn_sha256_transform);
    secp256k1_sha256_finalize(&sha, output, ctx->hash_context.fn_sha256_transform);
...
int secp256k1_ellswift_xdh(const secp256k1_context *ctx, unsigned char *output, const unsigned char *ell_a64, const unsigned char *ell_b64, const unsigned char *seckey32, int party, secp256k1_ellswift_xdh_hash_function hashfp, void *data) {
    ...
    if (hashfp == secp256k1_ellswift_xdh_hash_function_bip324 && ctx->hash_context.fn_sha256_transform != NULL) {
        ret = ellswift_xdh_hash_function_bip324_impl(ctx, output, sx, ell_a64, ell_b64, data);
    } else if (hashfp == secp256k1_ellswift_xdh_hash_function_prefix && ctx->hash_context.fn_sha256_transform != NULL) {
