...
static int secp256k1_musig_compute_pks_hash(const secp256k1_context *ctx, unsigned char *pks_hash, const secp256k1_pubkey * const* pks, size_t np) {
    ...
    for (i = 0; i < np; i++) {
        ...
        secp256k1_sha256_write(&sha, ser, sizeof(ser), ctx->hash_context.fn_sha256_transform);
    }
    secp256k1_sha256_finalize(&sha, pks_hash, ctx->hash_context.fn_sha256_transform);
    return 1;
}
...
static void secp256k1_musig_keyaggcoef_internal(const secp256k1_context *ctx, secp256k1_scalar *r, const unsigned char *pks_hash, secp256k1_ge *pk, const secp256k1_ge *second_pk) {
    ...
    if (!secp256k1_ge_is_infinity(second_pk)
        ...
        secp256k1_sha256_write(&sha, pks_hash, 32, ctx->hash_context.fn_sha256_transform);
