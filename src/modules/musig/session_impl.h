...
static void secp256k1_nonce_function_musig_helper(const secp256k1_context *ctx, secp256k1_sha256 *sha, unsigned int prefix_size, const unsigned char *data, unsigned char len) {
    ...
    secp256k1_sha256_write(sha, zero, prefix_size - 1, secp256k1_get_sha256_transform(ctx));
    if (data != NULL) {
        secp256k1_sha256_write(sha, &len, 1, secp256k1_get_sha256_transform(ctx));
        secp256k1_sha256_write(sha, data, len, secp256k1_get_sha256_transform(ctx));
    } else {
        len = 0;
        secp256k1_sha256_write(sha, &len, 1, secp256k1_get_sha256_transform(ctx));
    }
...
static void secp256k1_nonce_function_musig(const secp256k1_context *ctx, secp256k1_scalar *k, const unsigned char *session_secrand, const unsigned char *msg32, const unsigned char *seckey32, const unsigned char *pk33, const unsigned char *agg_pk32, const unsigned char *extra_input32) {
    ...
    if (seckey32 != NULL) {
        ...
        secp256k1_sha256_write(&sha, session_secrand, 32, secp256k1_get_sha256_transform(ctx));
        secp256k1_sha256_finalize(&sha, rand, secp256k1_get_sha256_transform(ctx));
        for (i = 0; i < 32; i++) {
            rand[i] ^= seckey32[i];
        }
    } else {
    ...
    secp256k1_sha256_write(&sha, rand, sizeof(rand), secp256k1_get_sha256_transform(ctx));
    ...
    secp256k1_sha256_write(&sha, &msg_present, 1, secp256k1_get_sha256_transform(ctx));
    if (msg_present) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        secp256k1_sha256_write(&sha_tmp, &i, 1, secp256k1_get_sha256_transform(ctx));
        secp256k1_sha256_finalize(&sha_tmp, buf, secp256k1_get_sha256_transform(ctx));
...
static void secp256k1_musig_compute_noncehash(const secp256k1_context *ctx, unsigned char *noncehash, secp256k1_ge *aggnonce, const unsigned char *agg_pk32, const unsigned char *msg) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        secp256k1_sha256_write(&sha, buf, sizeof(buf), secp256k1_get_sha256_transform(ctx));
    }
    secp256k1_sha256_write(&sha, agg_pk32, 32, secp256k1_get_sha256_transform(ctx));
    secp256k1_sha256_write(&sha, msg, 32, secp256k1_get_sha256_transform(ctx));
    secp256k1_sha256_finalize(&sha, noncehash, secp256k1_get_sha256_transform(ctx));
}
