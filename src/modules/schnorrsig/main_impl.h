...
static int nonce_function_bip340_impl(const secp256k1_context *ctx, unsigned char *nonce32, const unsigned char *msg, size_t msglen, const unsigned char *key32, const unsigned char *xonly_pk32, const unsigned char *algo, size_t algolen, void *data) {
    ...
    if (data != NULL) {
        ...
        secp256k1_sha256_write(&sha, data, 32, secp256k1_get_sha256_transform(ctx));
        secp256k1_sha256_finalize(&sha, masked_key, secp256k1_get_sha256_transform(ctx));
        for (i = 0; i < 32; i++) {
    ...
    if (algolen == sizeof(bip340_algo)
        ...
        secp256k1_sha256_initialize_tagged(&sha, algo, algolen, secp256k1_get_sha256_transform(ctx));
    }

    /* Hash masked-key||pk||msg using the tagged hash as per the spec */
    secp256k1_sha256_write(&sha, masked_key, 32, secp256k1_get_sha256_transform(ctx));
    secp256k1_sha256_write(&sha, xonly_pk32, 32, secp256k1_get_sha256_transform(ctx));
    secp256k1_sha256_write(&sha, msg, msglen, secp256k1_get_sha256_transform(ctx));
    secp256k1_sha256_finalize(&sha, nonce32, secp256k1_get_sha256_transform(ctx));
...
static void secp256k1_schnorrsig_challenge(const secp256k1_context *ctx, secp256k1_scalar* e, const unsigned char *r32, const unsigned char *msg, size_t msglen, const unsigned char *pubkey32)
{
    ...
    secp256k1_sha256_write(&sha, r32, 32, secp256k1_get_sha256_transform(ctx));
    secp256k1_sha256_write(&sha, pubkey32, 32, secp256k1_get_sha256_transform(ctx));
    secp256k1_sha256_write(&sha, msg, msglen, secp256k1_get_sha256_transform(ctx));
    secp256k1_sha256_finalize(&sha, buf, secp256k1_get_sha256_transform(ctx));
