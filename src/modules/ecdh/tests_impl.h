...
static void test_ecdh_generator_basepoint_impl(secp256k1_context *ctx) {
    ...
    for (i = 0; i < COUNT; ++i) {
        ...
        secp256k1_sha256_write(&sha, point_ser, point_ser_len, ctx->hash_context.fn_sha256_transform);
        secp256k1_sha256_finalize(&sha, output_ser, ctx->hash_context.fn_sha256_transform);
        /* compare */
